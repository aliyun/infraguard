package infraguard.rules.terraform.ecs_instance_not_bind_key_pair
...
has_key_pair(resource) if {
	key_name := tf.get_attribute(resource, "key_name", "")
	not tf.is_unknown(key_name)
	key_name != ""
}

has_key_pair(resource) if {
	key_pair_name := tf.get_attribute(resource, "key_pair_name", "")
	not tf.is_unknown(key_pair_name)
	key_pair_name != ""
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_instance")
	not has_key_pair(resource)
