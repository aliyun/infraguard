package infraguard.rules.terraform.fc_function_custom_domain_and_tls_enable
...
rule_meta := {
	...
	"severity": "medium",
	"name": {
		"en": "FC Function Custom Domain TLS Enabled",
...
has_cert_config(resource) if {
	cert_config := tf.get_attribute(resource, "cert_config", {})
	not tf.is_unknown(cert_config)
	cert_config != {}
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_fc_custom_domain")
	not has_cert_config(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_fc_custom_domain.%s", [name]),
		"violation_path": ["cert_config"],
