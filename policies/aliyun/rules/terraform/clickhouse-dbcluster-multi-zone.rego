package infraguard.rules.terraform.clickhouse_dbcluster_multi_zone
...
rule_meta := {
	...
	"resource_types": ["alicloud_click_house_db_cluster"],
	"iac_type": "terraform"
}

# Check if cluster is high availability (multi-zone)
is_high_availability(resource) if {
	category := tf.get_attribute(resource, "category", "")
	not tf.is_unknown(category)
	category == "HighAvailability"
}

# Deny rule: ClickHouse clusters should use HighAvailability edition
deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_click_house_db_cluster")
	not is_high_availability(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_click_house_db_cluster.%s", [name]),
