package infraguard.rules.terraform.waf3_defense_resource_logging_enabled
...
rule_meta := {
	...
	"recommendation": {
		"en": "Enable log service for the WAF 3.0 instance by setting `log_status` to true.",
		"zh": "通过将 `log_status` 设置为 true 为 WAF 3.0 实例开启日志服务。",
		"ja": "`log_status` を true に設定して WAF 3.0 インスタンスのログサービスを有効にします。",
		"de": "Aktivieren Sie den Log-Service für die WAF 3.0-Instanz, indem Sie `log_status` auf true setzen.",
		"es": "Habilite el servicio de registro para la instancia WAF 3.0 estableciendo `log_status` en true.",
		"fr": "Activez le service de journalisation pour l'instance WAF 3.0 en définissant `log_status` sur true.",
		"pt": "Habilite o serviço de registro para a instância WAF 3.0 definindo `log_status` como true."
	},
	"resource_types": ["alicloud_wafv3_instance"],
	"iac_type": "terraform"
}

is_compliant(resource) if {
	status := tf.get_attribute(resource, "log_status", false)
	not tf.is_unknown(status)
	status == true
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_wafv3_instance")
	not is_compliant(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_wafv3_instance.%s", [name]),
