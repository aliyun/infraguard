package infraguard.rules.terraform.cr_instance_any_ip_access_check
...
rule_meta := {
	...
	"resource_types": ["alicloud_cr_endpoint_acl_policy"],
	"iac_type": "terraform"
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_cr_endpoint_acl_policy")
	tf.get_attribute(resource, "entry", "") == "0.0.0.0/0"
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_cr_endpoint_acl_policy.%s", [name]),
		"violation_path": ["entry"],
