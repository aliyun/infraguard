package infraguard.rules.terraform.alb_acl_public_access_check
...
rule_meta := {
	...
	"resource_types": ["alicloud_alb_acl"],
	"iac_type": "terraform"
}
...
contains_public_cidr(resource) if {
	entries := as_array(tf.get_attribute(resource, "acl_entries", []))
...
deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_alb_acl")
	contains_public_cidr(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_alb_acl.%s", [name]),
