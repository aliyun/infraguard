package infraguard.rules.terraform.sg_risky_ports_check
...
rule_meta := {
	...
	"resource_types": ["alicloud_security_group_rule"],
	"iac_type": "terraform"
}
...
deny contains violation if {
	...
	tf.get_attribute(resource, "type", "") == "ingress"
	cidr := tf.get_attribute(resource, "cidr_ip", "")
	not tf.is_unknown(cidr)
	cidr == "0.0.0.0/0"
	port_range := tf.get_attribute(resource, "port_range", "")
	not tf.is_unknown(port_range)
	is_risky_port(port_range)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_security_group_rule.%s", [name]),
		"meta": {
