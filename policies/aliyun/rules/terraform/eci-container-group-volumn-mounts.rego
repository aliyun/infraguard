package infraguard.rules.terraform.eci_container_group_volumn_mounts
...
has_volume(resource) if {
	volumes := as_blocks(tf.get_attribute(resource, "volumes", []))
	count(volumes) > 0
}

has_volume_mount_in(containers) if {
	some container in as_blocks(containers)
	mounts := as_blocks(tf.get_attribute(container, "volume_mounts", []))
	count(mounts) > 0
}

has_volume_mount(resource) if {
	has_volume_mount_in(tf.get_attribute(resource, "containers", []))
}

has_volume_mount(resource) if {
	has_volume_mount_in(tf.get_attribute(resource, "init_containers", []))
}

is_compliant(resource) if {
	has_volume(resource)
	has_volume_mount(resource)
}
