package infraguard.rules.terraform.api_gateway_api_internet_request_https
...
request_protocol(resource) := object.get(as_object(tf.get_attribute(resource, "request_config", {})), "request_protocol", "")

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_api_gateway_api")
	request_protocol(resource) == "HTTP"
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_api_gateway_api.%s", [name]),
		"violation_path": ["request_config", "request_protocol"],
