package infraguard.rules.terraform.ess_scaling_configuration_attach_security_group
...
rule_meta := {
	...
	"description": {
		"en": "ESS scaling configurations should attach security groups.",
	...
	"reason": {
		"en": "The ESS scaling configuration does not have security groups attached.",
	...
	"recommendation": {
		"en": "Set security_group_id or security_group_ids.",
		"zh": "在伸缩配置中配置安全组。",
...
has_security_group(resource) if {
	security_group_ids := tf.get_attribute(resource, "security_group_ids", [])
	not tf.is_unknown(security_group_ids)
