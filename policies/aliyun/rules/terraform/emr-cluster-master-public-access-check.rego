package infraguard.rules.terraform.emr_cluster_master_public_access_check
...
rule_meta := {
	...
	"description": {
		...
		"pt": "Os nós mestres do EMR no ECS não devem ter IP público habilitado."
	},
	"reason": {
	...
	"recommendation": {
		"en": "Set 'is_open_public_ip' to false for the EMR cluster and use a NAT gateway or bastion host for access.",
		"zh": "将 EMR 集群的'is_open_public_ip'属性设置为 false，并使用 NAT 网关或堡垒机进行访问。",
		"ja": "EMR クラスターの 'is_open_public_ip' を false に設定し、アクセスに NAT ゲートウェイまたはバスチオンホストを使用します。",
		"de": "Setzen Sie 'is_open_public_ip' für den EMR-Cluster auf false und verwenden Sie ein NAT-Gateway oder einen Bastion-Host für den Zugriff.",
		"es": "Establezca 'is_open_public_ip' en false para el clúster EMR y use una puerta de enlace NAT o un host bastión para el acceso.",
		"fr": "Définissez 'is_open_public_ip' sur false pour le cluster EMR et utilisez une passerelle NAT ou un hôte bastion pour l'accès.",
		"pt": "Defina 'is_open_public_ip' como false para o cluster EMR e use um gateway NAT ou host bastião para acesso."
	},
	"resource_types": ["alicloud_emr_cluster"],
	"iac_type": "terraform"
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_emr_cluster")
	is_open_public_ip := tf.get_attribute(resource, "is_open_public_ip", false)
	not tf.is_unknown(is_open_public_ip)
	is_open_public_ip == true
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_emr_cluster.%s", [name]),
		"violation_path": ["is_open_public_ip"],
		"meta": {
