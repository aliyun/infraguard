package infraguard.rules.terraform.ack_cluster_log_plugin_installed
...
# Check if log-service addon is installed (addons as list)
has_log_service(resource) if {
	addons := tf.get_attribute(resource, "addons", [])
	is_array(addons)
	some addon in addons
	addon.name == "logtail-ds"
}

# Check if log-service addon is installed (single addon block stored as map)
has_log_service(resource) if {
	addons := tf.get_attribute(resource, "addons", {})
	is_object(addons)
