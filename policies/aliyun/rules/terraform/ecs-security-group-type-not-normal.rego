package infraguard.rules.terraform.ecs_security_group_type_not_normal
...
rule_meta := {
	...
	"resource_types": ["alicloud_security_group"],
	"iac_type": "terraform"
}

is_enterprise_type(resource) if {
	security_group_type := tf.get_attribute(resource, "security_group_type", "")
	not tf.is_unknown(security_group_type)
...
deny contains violation if {
	...
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_security_group.%s", [name]),
		"meta": {
