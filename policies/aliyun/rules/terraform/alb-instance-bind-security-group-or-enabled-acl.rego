package infraguard.rules.terraform.alb_instance_bind_security_group_or_enabled_acl
...
has_security_groups(resource) if {
	sg_ids := tf.get_attribute(resource, "security_group_ids", [])
	not tf.is_unknown(sg_ids)
	count(sg_ids) > 0
}
