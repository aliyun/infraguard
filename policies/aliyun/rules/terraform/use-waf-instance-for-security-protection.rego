package infraguard.rules.terraform.use_waf_instance_for_security_protection
...
rule_meta := {
	...
	"recommendation": {
		"en": "Enable WAF for the ALB instance by setting `load_balancer_edition` to 'StandardWithWaf'.",
		"zh": "通过将 `load_balancer_edition` 设置为 'StandardWithWaf' 为 ALB 实例开启 WAF 防护。",
		"ja": "`load_balancer_edition` を 'StandardWithWaf' に設定して、ALB インスタンスの WAF を有効にします。",
		"de": "Aktivieren Sie WAF für die ALB-Instanz, indem Sie `load_balancer_edition` auf 'StandardWithWaf' setzen.",
		"es": "Habilite WAF para la instancia ALB estableciendo `load_balancer_edition` en 'StandardWithWaf'.",
		"fr": "Activez WAF pour l'instance ALB en définissant `load_balancer_edition` sur 'StandardWithWaf'.",
		"pt": "Habilite WAF para a instância ALB definindo `load_balancer_edition` como 'StandardWithWaf'."
	},
