package infraguard.rules.terraform.ess_scaling_configuration_system_disk_encrypted
...
rule_meta := {
	...
	"description": {
		"en": "ESS scaling configurations should enable system disk encryption.",
	...
	"recommendation": {
		"en": "Set system_disk_encrypted, system_disk_kms_key_id, or system_disk_encrypt_algorithm.",
		"zh": "在伸缩配置中启用系统磁盘加密功能。",
...
system_disk_encrypted(resource) if {
	tf.get_attribute(resource, "system_disk_encrypted", false) == true
}

system_disk_encrypted(resource) if {
	tf.get_attribute(resource, "system_disk_encrypted", "") == "true"
}

system_disk_encrypted(resource) if {
	algorithm := tf.get_attribute(resource, "system_disk_encrypt_algorithm", "")
	not tf.is_unknown(algorithm)
	algorithm != ""
}

system_disk_encrypted(resource) if {
	kms_key_id := tf.get_attribute(resource, "system_disk_kms_key_id", "")
...
deny contains violation if {
	...
	not system_disk_encrypted(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_ess_scaling_configuration.%s", [name]),
		"violation_path": ["system_disk_encrypted"],
