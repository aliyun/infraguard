package infraguard.rules.terraform.mse_gateway_multi_availability_area_architecture_check
...
rule_meta := {
	...
	"description": {
		"en": "MSE gateway should have backup_vswitch_id configured for multi-availability zone deployment.",
		"zh": "MSE 网关应配置 backup_vswitch_id 以实现多可用区部署。",
	...
	"reason": {
		"en": "The MSE gateway does not have backup_vswitch_id configured.",
		"zh": "MSE 网关未配置 backup_vswitch_id。",
	...
	"recommendation": {
		"en": "Set backup_vswitch_id to a VSwitch in a different availability zone.",
		"zh": "将 backup_vswitch_id 设置为不同可用区的 VSwitch。",
...
has_backup_vswitch(resource) if {
	backup_vswitch_id := tf.get_attribute(resource, "backup_vswitch_id", "")
	not tf.is_unknown(backup_vswitch_id)
	backup_vswitch_id != ""
}
