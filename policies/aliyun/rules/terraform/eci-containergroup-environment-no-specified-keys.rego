package infraguard.rules.terraform.eci_containergroup_environment_no_specified_keys
...
rule_meta := {
	...
	"resource_types": ["alicloud_eci_container_group"],
	"iac_type": "terraform"
}
...
get_sensitive_keys := input.rule_parameters.sensitive_env_keys if {
	keys := input.rule_parameters.sensitive_env_keys
	is_array(keys)
	count(keys) > 0
} else := sensitive_keys

as_blocks(value) := value if {
	is_array(value)
} else := [value] if {
	is_object(value)
} else := []
...
has_sensitive_env(container) if {
	some env in as_blocks(tf.get_attribute(container, "environment_vars", []))
	key := tf.get_attribute(env, "key", "")
	not tf.is_unknown(key)
	is_sensitive_key(key)
}

has_sensitive_env_in(containers) if {
	some container in as_blocks(containers)
	has_sensitive_env(container)
}

resource_has_sensitive_env(resource) if {
	has_sensitive_env_in(tf.get_attribute(resource, "containers", []))
}

resource_has_sensitive_env(resource) if {
	has_sensitive_env_in(tf.get_attribute(resource, "init_containers", []))
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_eci_container_group")
	resource_has_sensitive_env(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_eci_container_group.%s", [name]),
		"violation_path": ["containers", "environment_vars"],
		"meta": {
