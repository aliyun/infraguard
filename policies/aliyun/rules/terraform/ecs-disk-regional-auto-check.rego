package infraguard.rules.terraform.ecs_disk_regional_auto_check
...
rule_meta := {
	...
	"description": {
		"en": "ECS data disks should use zone-redundant ESSD storage for high availability.",
		"zh": "使用同城冗余类型的 ESSD 数据盘，视为合规。",
	...
	"recommendation": {
		"en": "Use zone-redundant ESSD storage by setting category to cloud_regional_disk_auto.",
		"zh": "通过将 category 设置为 cloud_regional_disk_auto 来使用同城冗余 ESSD 存储。",
...
is_zone_redundant(resource) if {
	tf.get_attribute(resource, "category", "") == "cloud_regional_disk_auto"
}
