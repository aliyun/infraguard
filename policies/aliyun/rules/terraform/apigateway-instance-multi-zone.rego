package infraguard.rules.terraform.apigateway_instance_multi_zone
...
rule_meta := {
	...
	"recommendation": {
		"en": "Deploy the API Gateway instance in a multi-zone configuration by specifying a ZoneId with MAZ format.",
		"zh": "通过指定 MAZ（多可用区）格式的 ZoneId，将 API 网关实例部署在多可用区配置中。",
	...
	"resource_types": ["alicloud_api_gateway_instance"],
	"iac_type": "terraform"
}
...
deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_api_gateway_instance")
	not is_multi_zone(tf.get_attribute(resource, "zone_id", ""))
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_api_gateway_instance.%s", [name]),
		"violation_path": ["zone_id"],
