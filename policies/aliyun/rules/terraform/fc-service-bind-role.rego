package infraguard.rules.terraform.fc_service_bind_role
...
rule_meta := {
	...
	"recommendation": {
		"en": "Bind a RAM role to the Function Compute service by setting the role attribute.",
		"zh": "通过设置 role 属性为函数计算服务绑定 RAM 角色。",
...
has_role(resource) if {
	role := tf.get_attribute(resource, "role", "")
	not tf.is_unknown(role)
	role != ""
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_fc_service")
	not has_role(resource)
	violation := {
		"id": rule_meta.id,
