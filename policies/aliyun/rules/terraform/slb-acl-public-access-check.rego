package infraguard.rules.terraform.slb_acl_public_access_check
...
rule_meta := {
	...
	"recommendation": {
		"en": "Remove 0.0.0.0/0 from the SLB ACL entry_list entries and replace it with specific IP ranges.",
		"zh": "从 CLB 访问控制列表 entry_list 条目中移除 0.0.0.0/0，并替换为特定的 IP 范围。",
		"ja": "SLB ACL entry_list エントリから 0.0.0.0/0 を削除し、特定の IP 範囲に置き換えます。",
		"de": "Entfernen Sie 0.0.0.0/0 aus den SLB ACL entry_list-Einträgen und ersetzen Sie es durch spezifische IP-Bereiche.",
		"es": "Elimine 0.0.0.0/0 de las entradas entry_list de ACL de SLB y reemplácelo con rangos de IP específicos.",
		"fr": "Supprimez 0.0.0.0/0 des entrées entry_list ACL SLB et remplacez-le par des plages d'IP spécifiques.",
		"pt": "Remova 0.0.0.0/0 das entradas entry_list de ACL SLB e substitua por intervalos de IP específicos."
	},
	"resource_types": ["alicloud_slb_acl"],
	"iac_type": "terraform"
}
...
has_public_entry(resource) if {
	entries := as_array(tf.get_attribute(resource, "entry_list", []))
	some entry in entries
	cidr := object.get(entry, "entry", "")
	cidr in {"0.0.0.0/0", "0.0.0.0"}
}

deny contains violation if {
	some name, resource in tf.resources_by_type("alicloud_slb_acl")
	has_public_entry(resource)
	violation := {
		"id": rule_meta.id,
		"resource_id": sprintf("alicloud_slb_acl.%s", [name]),
		"meta": {
